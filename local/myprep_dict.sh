...
#### My code is here
## The phonemes in the mySrcdict should be in the 60 Phonemes list.
cat $dir/myMRIdict.txt | tr '[A-Z]' '[a-z]' | sed "s/ sp//g" | $local/timit_norm_trans.pl -i - -m conf/myphones.60-48-40.map -to 40 | sort > $dir/mydict | sync || exit 1;
#cat $dir/mySrcdict | tr '[A-Z]' '[a-z]' | sed "s/ sp//g" | sort > $dir/mydict | sync || exit 1;

cat $dir/mydict | cut -d" " -f2- | tr " " "\n" | sort | uniq > $dir/myPhones
...
cat data/allTrans.txt | cut -d" " -f2- |sed "s/ (/=/g" | cut -d"=" -f1 | sed "s/) /=/g" | cut -d"=" -f2- | sed "s/'/XXXXX/g" | tr '[:punct:]' ' ' | tr -s " " | sed "s/XXXXX/'/g" | sed "s/ '/ /g" | tr '[A-Z]' '[a-z]' | \
